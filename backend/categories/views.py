...
from products.serializers import ProductWithActiveStockSerializer
...
class UserStockedCategoriesView(APIView):
    ...
    def get(self, request):
        ...
        categories = Category.objects.filter(
            products__stock_batches__user=user
...
class ShopCategoriesView(APIView):
    ...
    def get(self, request):
        categories = Category.objects.filter(products__stock_batches__batch_status='active').distinct().order_by('category_name')
        paginator = PageNumberPagination()
        result_page = paginator.paginate_queryset(categories, request)
        serializer = CategorySerializer(result_page, many=True)
        return paginator.get_paginated_response(serializer.data)
