...
class DashboardSummaryView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # === STOCK DATA ===
            # Determine user role and stock data
            is_manager = StockBatch.objects.filter(user=user).exists()
            is_shop_owner = ShopOwnerProducts.objects.filter(shop_owner=user).exists()
            
            if is_manager:
                total_stock_added = StockBatch.objects.filter(
                    user=user, batch_status='active'
                ).aggregate(total_quantity=Sum('quantity'))['total_quantity'] or 0
                source = "manager"
                
                # Manager-specific monthly data
                manager_stock_added = StockBatch.objects.filter(
                    user=user,
                    created_at__gte=start_date,
                    created_at__lt=end_date
                ).aggregate(total_quantity=Sum('original_quantity'))['total_quantity'] or 0
                
                manager_batches_count = StockBatch.objects.filter(
                    user=user,
                    created_at__gte=start_date,
                    created_at__lt=end_date
                ).count()
