...
import { getpurchasereport } from "../../services/reportservices";
...
import { getmyvendorservice } from "../../services/vendorservices";
...
const PurchaseReport = () => {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      ...
      try {
        const response = await getmyvendorservice();
        setVendors(response);
      } catch (error) {
  ...
  useEffect(() => {
    fetchPurchaseData();
  }, [selectedPeriod, selectedVendorId, startDate, endDate]);
  ...
  const prepareChartData = () => {
    if (!purchaseData?.data?.trends) {
      return {
        ...
        batchesSeries: [{ name: "Batches", data: [] }],
        categories: [],
      };
    }
    ...
    const batchesDataPoints = trends.map((trend) => trend.batches_count);
    ...
    return {
      ...
      batchesSeries: [{ name: "Batches", data: batchesDataPoints }],
      categories,
    };
  };
  ...
  return (
    <div>
      <PageMeta
        ...
        description="View detailed purchase reports, track spending, and analyze vendor performance."
      />
          ...
          <div className="mb-4">
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-center justify-between dark:bg-gray-800/50 dark:border-gray-700">
              <span className="text-gray-400 dark:text-gray-600 text-sm">
                Unable to load purchase report data
              </span>
              <button
                onClick={() => refetch()}
        ...
        <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
          <div className="w-full">
            ...
            <p className="mt-1 text-gray-500 text-theme-base dark:text-gray-400">
              {selectedPeriod === "day" && "Daily purchase analysis"}
              {selectedPeriod === "month" && "Monthly purchase analysis"}
              {selectedPeriod === "year" && "Annual purchase trends"}
              {purchaseData?.data?.vendor?.name &&
                ` - ${purchaseData.data.vendor.name}`}
            </p>
            ...
            {purchaseData?.data?.summary && !isLoading && (
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div className="bg-gray-50 rounded-lg p-3 dark:bg-gray-800/50">
                  ...
                  <p className="text-lg font-semibold text-gray-800 dark:text-white">
                    {purchaseData.data.summary.total_batches.toLocaleString(
                      "en-IN"
                    )}
                  </p>
                </div>
            ...
            <div className="min-w-[500px] xl:min-w-full">
              ...
              {isLoading ? (
                ...
                <Chart
                  ...
                  type="bar"
                  height={310}
                  key={`amount-${selectedPeriod}-${selectedVendorId}-${startDate}-${endDate}`}
                />
              )}
            </div>
          </div>
            ...
            <div className="min-w-[500px] xl:min-w-full">
              ...
              {isLoading ? (
                ...
                <Chart
                  ...
                  type="bar"
                  height={310}
                  key={`quantity-${selectedPeriod}-${selectedVendorId}-${startDate}-${endDate}`}
                />
              )}
            </div>
          </div>
