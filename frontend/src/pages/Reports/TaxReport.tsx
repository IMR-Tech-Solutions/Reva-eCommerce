...
import { gettaxreport } from "../../services/reportservices";
import { handleError } from "../../utils/handleError";
import { TaxReportResponse, TaxReportFilters } from "../../types/types2";
...
const TaxReport = () => {
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      const response = await gettaxreport(filters);
  ...
  // Prepare chart data using vendor breakdown
  const prepareChartData = () => {
    if (!reportData?.data?.vendor_breakdown) {
      return {
    ...
    const vendorBreakdown = reportData.data.vendor_breakdown;
    const categories = vendorBreakdown.map((vendor) => vendor.vendor_name);
    const taxData = vendorBreakdown.map((vendor) => vendor.tax_paid);
    const batchesData = vendorBreakdown.map((vendor) => vendor.batches_count);
  ...
  return (
    <div>
      <PageMeta
        title="Tax Report"
        description="View detailed tax reports, track tax payments, and analyze tax obligations."
      />
          ...
          <div className="mb-4">
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-center justify-between dark:bg-gray-800/50 dark:border-gray-700">
              <span className="text-gray-400 dark:text-gray-600 text-sm">
                Unable to load tax report data
              </span>
              <button
                onClick={() => refetch()}
        ...
        <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
          <div className="w-full">
            ...
            <p className="mt-1 text-gray-500 text-theme-base dark:text-gray-400">
              {selectedPeriod === "day" && "Daily tax analysis"}
              {selectedPeriod === "month" && "Monthly tax analysis"}
              {selectedPeriod === "year" && "Annual tax trends"}
            </p>
            ...
            <div className="min-w-[500px] xl:min-w-full">
              ...
              {isLoading ? (
                ...
                <Chart
                  ...
                  type="bar"
                  height={310}
                  key={`tax-${selectedPeriod}-${startDate}-${endDate}`}
                />
              )}
            </div>
          </div>
            ...
            <div className="min-w-[500px] xl:min-w-full">
              ...
              {isLoading ? (
                ...
                <Chart
                  ...
                  type="bar"
                  height={310}
                  key={`batches-${selectedPeriod}-${startDate}-${endDate}`}
                />
              )}
            </div>
          </div>
