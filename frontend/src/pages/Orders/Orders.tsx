...
const Orders = () => {
  ...
  const pageSize = 10;
  const [updatePosOrder, setUpdatePosOrder] = useState<POSOrderPayload | null>(
    null
  );
  ...
  const handleEdit = async (posorderID: number) => {
    try {
      const OrderData = await getsingleposorderservice(posorderID);
      setUpdatePosOrder(OrderData);
      setModalVisible(true);
    } catch (error) {
      console.error("Unable Update order:", error);
      handleError(error);
    }
  };
