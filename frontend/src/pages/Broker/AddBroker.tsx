...
const AddBroker = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (const field of requiredFields) {
      if (!brokerData[field as keyof typeof brokerData]) {
        toast.error(`${field.replace("_", " ")} is required.`);
        return;
      }
    }

    if (parseFloat(brokerData.default_commission_percent) < 0) {
      toast.error("Commission percentage cannot be negative.");
      return;
    }
    if (parseFloat(brokerData.default_commission_percent) > 100) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-3">
            ...
            <div>
              ...
              <input
                ...
                value={brokerData.default_commission_percent}
                onChange={handleChange}
                className="input-field"
                min="0"
                max="100"
