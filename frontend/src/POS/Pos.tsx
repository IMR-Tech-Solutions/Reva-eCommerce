...
const Pos = () => {
  ...
  const addToCart = (product: ProductData) => {
    if (product.active_stock.quantity <= 0) {
    ...
    setCart((prev) => {
      ...
      if (exists) {
        if (exists.quantity < product.active_stock.quantity) {
  ...
  const subtotal = cart.reduce(
    (acc, item) => acc + item.active_stock.selling_price * item.quantity,
    0
  );
  ...
  // Main content - only renders when data is loaded and available
  return (
    <>
      <PageMeta
        title="POS"
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs theme-text">
                        Item Amount:{" "}
                        {item.active_stock.selling_price * item.quantity}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        className="!bg-white/[0.1] text-black dark:!text-white border dark:!border-gray-500"
                        onClick={() =>
                          updateQuantity(item.id, 1, item.active_stock.quantity)
                        }
                      >
                        +
                      </Button>
