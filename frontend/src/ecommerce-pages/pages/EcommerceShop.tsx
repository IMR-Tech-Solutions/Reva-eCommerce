...
const EcommerceShop = () => {
  ...
  const [price, setPrice]                         = useState<number>(
    () => Math.max(...products.map((p) => p.price))       // ← lazy init fix
  );
  ...
  const clearAllFilters = () => {
    ...
    setPrice(Math.max(...products.map((p) => p.price)));  // ← fix
    setSearchTerm("");
    setCurrentPage(1);
  };
  ...
  const SidebarContent = () => (
    <div className="flex flex-col">
      ...
      <FilterBlock
        title="Categories"
        isOpen={openBlocks.category}
        onToggle={() => toggleBlock("category")}
      ...
      >
        {categories.map((cat) => (
          <RadioOption
            ...
            onClick={() =>
              handleFilter(setSelectedCategory)(selectedCategory === cat ? null : cat)
            }
          />
        ))}
      </FilterBlock>
      ...
      <FilterBlock
        title="Material"
        isOpen={openBlocks.material}
        onToggle={() => toggleBlock("material")}
        activeCount={selectedMaterial ? 1 : 0}
      >
        {materials.map((m) => (
          <RadioOption
            key={m}
            ...
            onClick={() =>
              handleFilter(setSelectedMaterial)(selectedMaterial === m ? null : m)
            }
          />
        ))}
      </FilterBlock>
      ...
      <FilterBlock
        title="Pressure"
        isOpen={openBlocks.pressure}
        onToggle={() => toggleBlock("pressure")}
        activeCount={selectedPressure ? 1 : 0}
      >
        {pressures.map((p) => (
          <RadioOption
            key={p}
            ...
            onClick={() =>
              handleFilter(setSelectedPressure)(selectedPressure === p ? null : p)
            }
          />
        ))}
      </FilterBlock>
      ...
      <FilterBlock
        title="Flow Rate"
        isOpen={openBlocks.flow}
        onToggle={() => toggleBlock("flow")}
        activeCount={selectedFlowRate ? 1 : 0}
      >
        {flowRates.map((f) => (
          <RadioOption
            key={f}
            ...
            onClick={() =>
              handleFilter(setSelectedFlowRate)(selectedFlowRate === f ? null : f)
            }
          />
        ))}
      </FilterBlock>
      ...
      <FilterBlock
        title="Motor HP"
        isOpen={openBlocks.motor}
        onToggle={() => toggleBlock("motor")}
        activeCount={selectedMotorHP ? 1 : 0}
      >
        {motorHPs.map((hp) => (
          <RadioOption
            key={hp}
            ...
            onClick={() =>
              handleFilter(setSelectedMotorHP)(selectedMotorHP === hp ? null : hp)
            }
          />
        ))}
      </FilterBlock>
          ...
          <div className="bg-white border border-gray-200 rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3 shadow-sm">
            ...
            <div className="flex items-center gap-3 flex-shrink-0 flex-wrap">
              <p className="text-sm text-gray-500 hidden sm:block whitespace-nowrap">
                <span className="font-black text-[#1C1C1E]">{filteredProducts.length}</span>
                {" "}results
              </p>
              <div className="flex items-center gap-2">
                      ...
                      <div className="p-4 flex flex-col gap-3 flex-1">
                        ...
                        <div className="mt-auto pt-3 flex items-center justify-between border-t border-gray-100">
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 text-xs font-black uppercase tracking-wide text-[#1C1C1E] bg-[#FFB700] hover:bg-[#FFC933] hover:shadow-md hover:shadow-[#FFB700]/20 px-3.5 py-2.5 rounded-lg transition-all duration-200"
                          >
