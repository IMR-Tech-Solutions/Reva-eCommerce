...
import { getadmintaxreport } from "../../services/reportservices";
import { getallusersservice } from "../../services/newuserservices";
import { handleError } from "../../utils/handleError";
import {
  AdminTaxReportResponse,
...
const AdminTaxReport = () => {
  ...
  const [reportData, setReportData] = useState<AdminTaxReportResponse | null>(
    null
  );
  ...
  // Filter active users from tax report data
  useEffect(() => {
    if (reportData?.data?.user_breakdown && users.length > 0) {
      const activeUserIds = reportData.data.user_breakdown.map(
        (user) => user.user_id
      );
  ...
  const fetchReportData = async () => {
    ...
    setIsError(false);
    try {
      const filters: AdminTaxReportFilters = {
      ...
      const response = await getadmintaxreport(filters);
      setReportData(response);
    } catch (error) {
      setIsError(true);
      handleError(error);
      console.error("Error fetching admin tax report data:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Prepare chart data using user breakdown
  const prepareChartData = () => {
    if (!reportData?.data?.user_breakdown) {
      return {
        taxSeries: [{ name: "Tax Paid", data: [] }],
        batchesSeries: [{ name: "Batches Count", data: [] }],
        categories: [],
      };
    }

    const userBreakdown = reportData.data.user_breakdown;
    const categories = userBreakdown.map((user) => user.user_name);
    const taxData = userBreakdown.map((user) => user.tax_paid);
    const batchesData = userBreakdown.map((user) => user.batches_count);

    return {
      taxSeries: [{ name: "Tax Paid", data: taxData }],
      batchesSeries: [{ name: "Batches Count", data: batchesData }],
      categories,
    };
  };

  const { taxSeries, batchesSeries, categories } = prepareChartData();

  // Chart options for Tax
  const taxOptions: ApexOptions = {
    colors: ["#3B82F6"],
    chart: {
  ...
  const batchColumns: ColumnsType<any> = [
...

    {
      ...
      sorter: (a, b) => a.original_quantity - b.original_quantity,
    },
    {
    ...
    {
      title: "Tax %",
      dataIndex: "tax_percentage_of_purchase",
      key: "tax_percentage_of_purchase",
      render: (percent: number) => `${percent.toFixed(2)}%`,
      sorter: (a, b) =>
        a.tax_percentage_of_purchase - b.tax_percentage_of_purchase,
    },
    {
      title: "Tax Amount",
  ...
  return (
    <div>
      <PageMeta
        title="Admin Tax Report"
        description="View detailed tax reports across all users, track tax payments, and analyze tax obligations."
      />

      <PageBreadcrumb pageTitle="Admin Tax Report" />
      ...
      <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
          ...
          <div className="mb-4">
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-center justify-between dark:bg-gray-800/50 dark:border-gray-700">
              <span className="text-gray-400 dark:text-gray-600 text-sm">
                Unable to load admin tax report data
              </span>
              <button
                onClick={() => refetch()}
        ...
        <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
          <div className="w-full">
            ...
            <p className="mt-1 text-gray-500 text-theme-base dark:text-gray-400">
              {selectedPeriod === "day" && "Daily tax analysis across system"}
              {selectedPeriod === "month" &&
                "Monthly tax analysis across system"}
              {selectedPeriod === "year" && "Annual tax trends across system"}
              {reportData?.data?.target_user_name &&
                ` - ${reportData.data.target_user_name}`}
            </p>
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="bg-gray-50 rounded-lg p-3 dark:bg-gray-800/50">
                  ...
                  <p className="text-lg font-semibold text-gray-800 dark:text-white">
                    {formatINR(reportData.data.summary.total_tax_paid)}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3 dark:bg-gray-800/50">
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <p className="text-lg font-semibold text-gray-800 dark:text-white">
                    {reportData.data.summary.total_batches_with_tax.toLocaleString(
                      "en-IN"
                    )}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3 dark:bg-gray-800/50">
                  ...
                  <p className="text-lg font-semibold text-gray-800 dark:text-white">
                    {formatINR(reportData.data.summary.total_purchase_value)}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3 dark:bg-gray-800/50">
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <p className="text-lg font-semibold text-gray-800 dark:text-white">
                    {reportData.data.summary.tax_percentage_of_purchases.toFixed(
                      2
                    )}
                    %
                  </p>
            ...
            {/* Top Users */}
            {reportData?.data?.user_breakdown &&
              !selectedUserId &&
              !isLoading && (
                <div className="mt-4">
                        ...
                        .map((user, index) => (
                          <div key={user.user_id} className="text-center">
                            ...
                            <p className="text-sm font-semibold text-gray-800 dark:text-white">
                              {formatINR(user.tax_paid)}
                            </p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              {user.batches_count} batches •{" "}
            ...
            <div className="min-w-[500px] xl:min-w-full">
              ...
              {isLoading ? (
                ...
                <Chart
                  ...
                  type="bar"
                  height={310}
                  key={`tax-${selectedPeriod}-${selectedUserId}-${startDate}-${endDate}`}
                />
              )}
            </div>
          </div>
            ...
            <div className="min-w-[500px] xl:min-w-full">
              ...
              {isLoading ? (
                ...
                <Chart
                  ...
                  type="bar"
                  height={310}
                  key={`batches-${selectedPeriod}-${selectedUserId}-${startDate}-${endDate}`}
                />
              )}
            </div>
          </div>
        ...
        >
          <Table
            ...
            scroll={{ x: 800 }}
            locale={{ emptyText: "No tax batch details found." }}
          />
        </ButtonComponentCard>
      </div>
    </div>
